...
fNom = [1,1,1,1]
fNames = [r"{FP2 resolution}${}^{-1}$",r"FP2 Espread vs xangle",r"FP3-res",r"FP3-e-xangle"]
...
#    ax.plot(color="C{}".format(j))
        ndf, dl, dc, ndl = pg.fast_non_dominated_sorting(popi.get_f()[0:j])
        new_ndf = np.array(ndf[0])
#        ax = pg.plot_non_dominated_fronts(popi.get_f()[0:j])
#        print(new_ndf,old_ndf)
        fig, ax = plt.subplots()
...
#                        print(popi.get_f()[i][0],popi.get_f()[sorted_ndf[j]][0],popi.get_f()[sorted_ndf[j-1]][0])
    ...
    for i in range(len(sorted_ndf)):
        j = sorted_ndf[i] 
        write_fox(np.power(np.zeros(magnet_dim)+2,popi.get_x()[j]), i, "2f_FP3_300/")
    return
